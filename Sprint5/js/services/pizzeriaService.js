...
export class PizzeriaService {
    ...
    static API_URL_LOCAL = 'http://127.0.0.1:8000/api/pizzeria';
    ...
    static async getAllDrinks() {
        const resposta = await fetch(`${this.API_URL}/beguda`);

        if (!resposta.ok) throw new Error('No s\'han pogut carregar les begudes');
        const dadesPG = await resposta.json();
        const dades = dadesPG.records;
        ...
        // Recorrem les dades i generem la llista de pizzes
        for (let i = 0; i < dades.length; i++) {
            const beguda = new Beguda(dades[i]);
            begudes.push(beguda);
        }
    ...
    static async sendOrder(comanda) {
        //const resposta = await fetch(`${this.API_URL_LOCAL}/order`, {
        const resposta = await fetch(`${this.API_URL}/order`, {
            method: 'POST',
            headers: {
        ...
        let js_resposta=await resposta.json();
